...
## PHASE X — Direct C/C++ SDK Bindings (Zero-Overhead Native Access)

> These SDKs link directly to `libinfer_api.so` — no HTTP, no JSON, no Go.
...
**What changes:**
- `sdk/c/README.md` — usage guide with all API examples
- `sdk/c/examples/chat.c` — minimal chat completion in pure C
- `sdk/c/examples/embed.c` — embedding + vector search
- `sdk/c/examples/detect.c` — object detection from JPEG
- `sdk/c/examples/rag.c` — full RAG pipeline in C
- `sdk/c/CMakeLists.txt` — `find_package(infergo)` integration
- `sdk/c/infergo.pc` — pkg-config file for easy linking

**Test cases:**

| ID | Test | Target | Result |
|---|---|---|---|
| OPT-136-T1 | chat.c compiles and runs | `gcc chat.c -linfer_api` → generates text | |
| OPT-136-T2 | CMake find_package works | `find_package(infergo)` → links correctly | |
| OPT-136-T3 | pkg-config works | `pkg-config --libs infergo` → correct flags | |
| OPT-136-T4 | All examples build | `make examples` → all 4 compile | |
| OPT-136-T5 | Performance matches raw | chat.c latency = raw_bench latency ±1ms | |
...
**What changes:**
- `sdk/python-native/infergo_native/__init__.py` — ctypes wrapper
- Auto-find `libinfer_api.so` (LD_LIBRARY_PATH, /usr/local/lib, ./build)
- Classes: `LLM`, `Embedding`, `Detector`, `VectorDB`, `BM25`
- NumPy integration: embeddings returned as `np.ndarray`
- Context manager: `with LLM("model.gguf") as llm:`
...
| ID | Test | Target | Result |
|---|---|---|---|
| OPT-137-T1 | LLM generate works | `llm.generate("Hi")` → text output | |
| OPT-137-T2 | Embedding returns ndarray | `shape == (384,)`, dtype float32 | |
| OPT-137-T3 | VectorDB CRUD | Insert → search → find → delete | |
| OPT-137-T4 | BM25 search | Insert docs → search → ranked results | |
| OPT-137-T5 | LoRA load/unload | Load adapter → generate → unload | |
| OPT-137-T6 | Performance vs HTTP | Direct call < 1ms, HTTP call > 4ms | |
| OPT-137-T7 | Memory cleanup | Context manager frees C resources on exit | |
| OPT-137-T8 | Error handling | Bad model path → clear Python exception | |
...
**What changes:**
- `sdk/rust/infergo-sys/` — raw FFI bindings (auto-generated by bindgen from infer_api.h)
- `sdk/rust/infergo/` — safe Rust wrapper with RAII, Result types, iterators
- `Cargo.toml` with build.rs for auto-linking
- Types: `Llm`, `Embedding`, `Detector`, `VectorDb`, `Bm25Index`
- All handles implement `Drop` for automatic cleanup
- `Send + Sync` where thread-safe

```rust
use infergo::{Llm, VectorDb};

let llm = Llm::load("model.gguf", -1, 4096)?;
let text = llm.generate("Hello", 32, 0.7)?;

let db = VectorDb::new(384);
...
| ID | Test | Target | Result |
|---|---|---|---|
| OPT-138-T1 | bindgen generates | `cargo build` in infergo-sys succeeds | |
| OPT-138-T2 | LLM generate | `llm.generate()` returns String | |
| OPT-138-T3 | RAII cleanup | Drop impl frees C handles (no leaks under valgrind) | |
| OPT-138-T4 | Error handling | Bad path → `Err(InfergoError::Load)` | |
| OPT-138-T5 | Vector search | Insert + search → correct results | |
| OPT-138-T6 | Thread safety | `Send + Sync` bounds compile, multi-thread test | |
| OPT-138-T7 | Batch generate | `llm.generate_batch()` processes N prompts | |
...
**What changes:**
- `sdk/java/native/infergo_jni.c` — JNI bridge (C → Java method mapping)
- `sdk/java/src/main/java/io/infergo/` — Java classes
- Classes: `InfergoLLM`, `InfergoEmbedding`, `InfergoDetector`, `InfergoSearch`
- Auto-load native library: `System.loadLibrary("infergo_jni")`
- Maven/Gradle packaging
- Kotlin extensions for coroutine support

```java
try (InfergoLLM llm = InfergoLLM.load("model.gguf", -1, 4096)) {
    String response = llm.generate("Hello", 32, 0.7f);
    System.out.println(response);
}

InfergoSearch db = new InfergoSearch(384);
db.insert(0, embeddingVector);
SearchResult[] results = db.search(queryVector, 10);
```

```kotlin
// Kotlin coroutine support
val llm = InfergoLLM.load("model.gguf")
val response = withContext(Dispatchers.IO) {
    llm.generate("Hello", maxTokens = 32)
}
```
...
| ID | Test | Target | Result |
|---|---|---|---|
| OPT-139-T1 | JNI loads | `System.loadLibrary("infergo_jni")` succeeds | |
| OPT-139-T2 | LLM generate | `llm.generate("Hi")` returns non-empty String | |
| OPT-139-T3 | AutoCloseable | try-with-resources frees native handles | |
| OPT-139-T4 | Vector search | Insert → search → correct results | |
| OPT-139-T5 | BM25 search | Keyword search returns ranked results | |
| OPT-139-T6 | Kotlin coroutines | `suspend fun generate()` works on IO dispatcher | |
| OPT-139-T7 | Maven artifact | `mvn package` produces .jar with JNI | |
| OPT-139-T8 | Android compatible | Builds for arm64-v8a target | |
...
**What changes:**
- `sdk/node-native/src/infergo.cc` — N-API C++ addon
- `sdk/node-native/lib/index.ts` — TypeScript declarations
- `node-gyp` build system with prebuilt binaries
- Classes: `LLM`, `Embedding`, `VectorDB`, `BM25`
- Async: all heavy ops return Promises (run on libuv thread pool)
- Streaming: `llm.generateStream()` returns AsyncIterator

```typescript
import { LLM, VectorDB } from '@infergo/native';

const llm = await LLM.load('model.gguf', { gpuLayers: -1 });
const text = await llm.generate('Hello', { maxTokens: 32 });
...
| ID | Test | Target | Result |
|---|---|---|---|
| OPT-140-T1 | Addon loads | `require('@infergo/native')` succeeds | |
| OPT-140-T2 | LLM generate | `llm.generate("Hi")` resolves to string | |
| OPT-140-T3 | Async non-blocking | Generate doesn't block event loop | |
| OPT-140-T4 | Streaming | AsyncIterator yields tokens one by one | |
| OPT-140-T5 | Vector search | Insert + search → correct results | |
| OPT-140-T6 | TypeScript types | Full type declarations, IDE autocomplete works | |
| OPT-140-T7 | Prebuilt binaries | `npm install` uses prebuilt, no compilation needed | |
| OPT-140-T8 | Memory management | GC ref prevents premature native free | |
...
**What changes:**
- `sdk/dotnet/Infergo/NativeMethods.cs` — P/Invoke declarations
- `sdk/dotnet/Infergo/` — managed wrapper classes
- Classes: `LlmModel`, `EmbeddingModel`, `VectorDatabase`, `Bm25Index`
- `IDisposable` pattern for native handle cleanup
- NuGet package: `dotnet add package Infergo`
- .NET 6+ and .NET Standard 2.1

```csharp
using Infergo;

using var llm = LlmModel.Load("model.gguf", gpuLayers: -1);
string response = llm.Generate("Hello", maxTokens: 32);

var db = new VectorDatabase(384);
db.Insert(0, embeddingVector);
var results = db.Search(queryVector, k: 10);
```
...
| ID | Test | Target | Result |
|---|---|---|---|
| OPT-141-T1 | P/Invoke loads | DllImport resolves libinfer_api | |
| OPT-141-T2 | LLM generate | `llm.Generate("Hi")` returns string | |
| OPT-141-T3 | IDisposable | `using` statement frees native handles | |
| OPT-141-T4 | Vector search | Insert + search → correct results | |
| OPT-141-T5 | NuGet package | `dotnet add package Infergo` installs | |
| OPT-141-T6 | Cross-platform | Works on Linux and Windows (.dll/.so) | |
...
**What changes:**
- `sdk/swift/Sources/Infergo/` — Swift package
- `sdk/swift/Sources/CInfergo/` — C bridging module (module.modulemap + infer_api.h)
- Classes: `LLMModel`, `EmbeddingModel`, `VectorDB`
- Swift-idiomatic: `throws`, `async/await`, `Sendable`
- SPM package: `.package(url: "https://github.com/ailakshya/infergo-swift")`

```swift
import Infergo

let llm = try LLMModel(path: "model.gguf", gpuLayers: -1)
let response = try await llm.generate("Hello", maxTokens: 32)

let db = VectorDB(dimensions: 384)
try db.insert(id: 0, vector: embedding)
let results = try db.search(query: queryVector, k: 10)
```
...
| ID | Test | Target | Result |
|---|---|---|---|
| OPT-142-T1 | SPM resolves | `swift build` compiles with C bridging | |
| OPT-142-T2 | LLM generate | `llm.generate()` returns String | |
| OPT-142-T3 | async/await | Runs on Swift concurrency without blocking | |
| OPT-142-T4 | Memory management | deinit calls infer_llm_destroy | |
| OPT-142-T5 | Vector search | Insert + search → correct results | |
| OPT-142-T6 | iOS compatible | Builds for arm64 iOS target | |
...
**What changes:**
- `sdk/ruby/lib/infergo.rb` — FFI bindings via `ffi` gem
- Classes: `Infergo::LLM`, `Infergo::VectorDB`, `Infergo::BM25`
- Gem: `gem install infergo`

```ruby
require 'infergo'

llm = Infergo::LLM.new("model.gguf", gpu_layers: -1)
puts llm.generate("Hello", max_tokens: 32)

db = Infergo::VectorDB.new(384)
db.insert(0, embedding)
...
| ID | Test | Target | Result |
|---|---|---|---|
| OPT-143-T1 | FFI loads | `require 'infergo'` succeeds | |
| OPT-143-T2 | LLM generate | `llm.generate("Hi")` returns string | |
| OPT-143-T3 | Vector search | Insert + search → correct results | |
| OPT-143-T4 | Gem packages | `gem build infergo.gemspec` succeeds | |
...
**What changes:**
- `sdk/php/src/Infergo.php` — PHP FFI bindings (PHP 7.4+ FFI)
- Classes: `Infergo\LLM`, `Infergo\VectorDB`, `Infergo\BM25`
- Composer package: `composer require infergo/infergo`

```php
use Infergo\LLM;
use Infergo\VectorDB;

$llm = new LLM("model.gguf", gpuLayers: -1);
echo $llm->generate("Hello", maxTokens: 32);
...
| ID | Test | Target | Result |
|---|---|---|---|
| OPT-144-T1 | FFI loads | `new LLM("model.gguf")` succeeds | |
| OPT-144-T2 | LLM generate | `$llm->generate("Hi")` returns string | |
| OPT-144-T3 | Vector search | Insert + search → correct results | |
| OPT-144-T4 | Composer package | `composer install` works | |
...
**What changes:**
- `sdk/dart/lib/infergo.dart` — dart:ffi bindings
- Classes: `InfergoLLM`, `InfergoVectorDB`
- `ffigen` for auto-generating bindings from infer_api.h
- Flutter plugin: `flutter pub add infergo`
- Async: all ops return `Future<T>` via Isolate

```dart
import 'package:infergo/infergo.dart';

final llm = await InfergoLLM.load('model.gguf', gpuLayers: -1);
final text = await llm.generate('Hello', maxTokens: 32);

final db = InfergoVectorDB(384);
db.insert(0, embedding);
final results = await db.search(query, k: 10);
```
...
| ID | Test | Target | Result |
|---|---|---|---|
| OPT-145-T1 | FFI loads | DynamicLibrary.open succeeds | |
| OPT-145-T2 | LLM generate | `llm.generate("Hi")` completes | |
| OPT-145-T3 | Isolate non-blocking | Inference doesn't freeze UI thread | |
| OPT-145-T4 | Flutter plugin | `flutter run` builds and runs | |
| OPT-145-T5 | Android + iOS | Builds for both platforms | |
...
**What changes:**
- `sdk/zig/src/infergo.zig` — Zig wrapper around C API
- Direct `@cImport` of infer_api.h
- Types: `Llm`, `VectorDb`, `Bm25Index`
- `build.zig` with system library linking
- Comptime error messages for missing library

```zig
const infergo = @import("infergo");

var llm = try infergo.Llm.init("model.gguf", .{ .gpu_layers = -1 });
defer llm.deinit();
const text = try llm.generate("Hello", .{ .max_tokens = 32 });
```
...
| ID | Test | Target | Result |
|---|---|---|---|
| OPT-146-T1 | Compiles | `zig build` succeeds with -linfer_api | |
| OPT-146-T2 | LLM generate | Generate returns valid string | |
| OPT-146-T3 | RAII cleanup | deinit frees C handles | |
| OPT-146-T4 | Vector search | Insert + search → correct results | |
...
**What changes:**
- `sdk/elixir/c_src/infergo_nif.c` — Erlang NIF implementation
- `sdk/elixir/lib/infergo.ex` — Elixir module
- Modules: `Infergo.LLM`, `Infergo.VectorDB`, `Infergo.BM25`
- Dirty scheduler for long-running inference (doesn't block BEAM)
- Hex package: `{:infergo, "~> 0.1"}`

```elixir
{:ok, llm} = Infergo.LLM.load("model.gguf", gpu_layers: -1)
{:ok, text} = Infergo.LLM.generate(llm, "Hello", max_tokens: 32)

{:ok, db} = Infergo.VectorDB.new(384)
:ok = Infergo.VectorDB.insert(db, 0, embedding)
{:ok, results} = Infergo.VectorDB.search(db, query, k: 10)
```
...
| ID | Test | Target | Result |
|---|---|---|---|
| OPT-147-T1 | NIF loads | `:infergo` application starts | |
| OPT-147-T2 | LLM generate | `Infergo.LLM.generate/3` returns `{:ok, text}` | |
| OPT-147-T3 | Dirty scheduler | Inference doesn't block other processes | |
| OPT-147-T4 | Vector search | Insert + search → correct results | |
| OPT-147-T5 | Hex package | `mix deps.get` installs | |
...
**What changes:**
- `sdk/lua/infergo.lua` — LuaJIT FFI bindings
- Functions: `infergo.llm_create`, `infergo.generate`, `infergo.search`
- Works with LuaJIT, Lua 5.1+ (via luaffi)

```lua
local infergo = require("infergo")

local llm = infergo.llm_create("model.gguf", -1, 4096)
local text = infergo.generate(llm, "Hello", 32, 0.7)
print(text)

infergo.llm_destroy(llm)
```
...
| ID | Test | Target | Result |
|---|---|---|---|
| OPT-148-T1 | FFI loads | `require("infergo")` succeeds in LuaJIT | |
| OPT-148-T2 | LLM generate | Generate returns string | |
| OPT-148-T3 | Memory cleanup | Destroy frees C handles | |
...
**What changes:**
- `sdk/wasm/` — Emscripten build of libinfer_api (CPU-only, no CUDA)
- `sdk/wasm/infergo.js` — JavaScript wrapper
- `sdk/wasm/infergo.d.ts` — TypeScript declarations
- Web Worker support for non-blocking inference
- IndexedDB model caching
- NPM package: `@infergo/wasm`

```javascript
import { InfergoWasm } from '@infergo/wasm';

const infergo = await InfergoWasm.init();
await infergo.loadModel('/models/qwen-0.5b-q4.gguf');
const text = await infergo.generate('Hello', { maxTokens: 32 });
```
...
| ID | Test | Target | Result |
|---|---|---|---|
| OPT-149-T1 | WASM compiles | Emscripten build produces .wasm + .js | |
| OPT-149-T2 | Browser loads | Model loads in Chrome/Firefox | |
| OPT-149-T3 | Generate works | Text generation in browser | |
| OPT-149-T4 | Web Worker | Inference doesn't block main thread | |
| OPT-149-T5 | Bundle size | WASM < 5MB (CPU-only, no CUDA) | |
| OPT-149-T6 | Performance | >= 5 tok/s for 0.5B model in browser | |

---

## Updated SDK task summary

| Phase | Tasks | Languages |
|---|---|---|
| X — Native SDK Bindings | OPT-136..149 | C/C++, Python, Rust, Java/Kotlin, TypeScript/Node, C#/.NET, Swift, Ruby, PHP, Dart/Flutter, Zig, Elixir/Erlang, Lua, WASM/JS |
| **Total new** | **14 tasks** | **14 languages** |
